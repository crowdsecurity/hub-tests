...
func main() {
	...
	if flags.SingleFile != "" {
		tested, failure := doTest(flags, flags.SingleFile, report)
		...
		for _, match := range matches {
			...
			tested, failure := doTest(flags, match, report)
			OverallResult.AddSingleResult(tested, failure)
	...
	for itemType, m := range overall {
		for item, testResult := range m {
			...
			report.AddSingleResult("Overall test", err, fmt.Sprintf("%s/%s %d times", itemType, item, testResult.count))
		}
	}
